...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    sum = Math.round(+sizeBlock.value * +materialBlock.value * 0.1 * +optionsBlock.value);
...
var drop = function drop() {
  ...
  // dragexit
  // dragleave - объект за пределами дропАреа
  // dragover - объект зависает над ДропАреа
  // dragstart
  // drop - объект упал в дропАреа
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("Couldnt fetch ".concat(url, ", status: ").concat(res.status));
